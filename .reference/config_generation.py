...
ENVIRONMENT_PATTERN = re.compile(r"(\w+): os\.getenv\((.*?)\)")
...
def parse_commands(raw_text: str) -> Dict[str, Any]:
    ...
    for command_match in command_matches:
        ...
        if func_match:
            ...
            environment_matches = ENVIRONMENT_PATTERN.findall(body)
            for name, env_details in environment_matches:
                commands[command_name]["options"][name] = {
                    "type": "str",
                    "help": env_details.strip()
                }
